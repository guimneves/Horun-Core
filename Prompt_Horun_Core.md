...
- **Onboarding no 1º acesso + busca global + anexos no Mural — implementado em 2026-09-10**:
  - **Onboarding**: `User.onboarded` (default False; a migração marca as contas já existentes como True pra não incomodar). `components/OnboardingModal.tsx` aparece uma vez no `Shell` quando `!user.onboarded` — foto + nome completo + e-mail + telefone; "Salvar e começar" ou "Pular por enquanto" (ambos marcam concluído, via `PATCH /auth/me` com `onboarded: true`). Assim o diretório não nasce vazio.
  - **Busca global** (`GET /search?q=`, `components/GlobalSearch.tsx` no lugar do placeholder do topo): varre avisos do Mural, equipamentos, módulos e colaboradores; resultados agrupados por tipo num dropdown, clicar navega. Debounce 220ms, mínimo 2 caracteres. Dados privados (telefone) nunca entram no resultado de pessoa. 7 testes (`tests/test_search.py`).
  - **Anexos no Mural**: um anexo por aviso (imagem JPEG/PNG/WEBP ou PDF, até 5 MB), bytes no banco (`Post.attachment*`, mesmo raciocínio da foto de perfil). `POST /posts` virou **multipart** (`content` + `file` opcional); `GET /posts/{id}/attachment` serve o arquivo (exige sessão). Composer com botão "Anexar" + chip do arquivo; `PostCard` mostra imagem embutida (`crossOrigin="use-credentials"` pro dev cross-origin, como o avatar) ou link pro PDF. 5 testes (`tests/test_posts.py`).
  - 109 testes no total, todos passando. Validado no navegador (modal de onboarding num usuário novo salvando e sumindo; busca agrupada navegando; anexo de imagem embutido e de PDF como link).

- **Migrações defensivas (bug real de deploy, 2026-09-10)**: o deploy de produção estava numa versão bem antiga; ao atualizar, várias colunas adicionadas depois do 1º deploy não existiam no Postgres e derrubavam login/módulos com 500. `_run_migrations()` (session.py) agora cobre `user` (colunas de perfil, com `_backfill_null_text` pros NULL legados) e `module` (`internal_frontend_url` etc. com DEFAULT; `DROP COLUMN codename`). `_out()` de usuário e de módulo coagem `None → ""` como cinto de segurança das rotas críticas. Lição: **toda coluna nova precisa de `_ensure_column` + o `_out` da rota tem que tolerar `None`**.

- **Grupos — Fase 1 (data de nascimento + aniversários + eventos do laboratório), implementado em 2026-09-10**. Alinhado antes: entrega em 3 fases; ano de nascimento opcional; mural de grupo aberto a qualquer membro; evento de grupo só pelo admin interno. Fase 1 não tem grupo ainda:
  - **Data de nascimento**: `User.birth_day/birth_month/birth_year` (3 inteiros, todos opcionais). Facilitador `components/BirthDateFields.tsx` — dois `<select>` (dia + mês por nome) + campo de ano opcional; mês por nome mata a confusão DD/MM, `<input type=date>` foi descartado (obriga ano). Autoatendimento no perfil e no onboarding (`PATCH /auth/me` com `birth_set`/`birth_day`/`birth_month`/`birth_year`). Validação: dia+mês juntos ou nenhum; 29/02 aceito; ano 1900..atual.
  - **Aniversários no calendário**: `GET /users/birthdays?start&end` — projeção dos perfis, **sem tabela** (29/02 → 28/02 em ano não bissexto). Na Agenda: faixa "dia inteiro" no topo de cada dia com chip 🎂 + widget "Aniversários da semana" na lateral. No Mural: widget "Próximos aniversários" (45 dias).
  - **Eventos do laboratório**: modelo `Event` novo (título, descrição, local, `all_day`, `start_at`/`end_at`) — **separado de `Reservation`** (sem regra de conflito, pode ser dia inteiro). `routes_events.py`: só o admin máximo cria/edita/remove (Fase 1), todo autenticado lista. Na Agenda: botão "Novo evento" (só admin), evento all-day na faixa do topo, evento com hora como bloco na grade (borda esquerda, visual distinto da reserva de equipamento), admin clica pra remover. `Event.group_id` fica pra Fase 2.
...
- **Ajustes pré-Fase 2 — implementado em 2026-09-10**:
  ...
  - **E-mail — decidido, a construir na Fase 2**: envio direto do backend via SMTP (não é serviço separado), em `BackgroundTasks` + pelo agendador. **SMTP: Gmail dedicado + senha de app** (`nqtr.horun@gmail.com` ou similar, 2FA + app password, `smtp.gmail.com:587`). Disparam e-mail: `@menção`, resposta ao seu aviso, digest semanal de aniversários. `User.email_notifications` (default true, precisa de e-mail preenchido) desliga por pessoa. `.env`: `SMTP_HOST/PORT/USER/PASS/FROM` — sem isso, e-mail fica inerte.

**Ainda não implementado**:

1. **Plugar os módulos da Juliana (Amostras, Reagentes)** — situação em 2026-09-10:
   - **Amostras (`juliana126/Controle-Analitico`) — patch aplicado por ela (commit `73543e1`), verificado**: `main.tsx` com `basename`, `api.ts` com `BASE_URL` em produção, `Dockerfile` com `ARG VITE_BASE`, serviço `amostras-frontend` no `docker-compose.yml` (na `horun-network`, sem `ports`), e link "← Voltar ao Horun" na barra lateral. Build com `--base=/m/amostras/` confere (assets em `/m/amostras/assets/`); backend lê `X-Horun-User-Id/User/Role` e com `HORUN_DEV_MODE=false` exige eles. **Falta só a ação no servidor** (`DESKTOP-N6KR7DO`): `git clone` do repo dela + `.env` + `docker compose up -d --build`, depois cadastrar no Core (Admin → Módulos: id `amostras`, backend `http://amostras-backend:8000`, frontend `http://amostras-frontend:80`) e conceder acesso.
   - **Reagentes (`juliana126/Controle-de-reagentes`) — ainda não**: último commit é um ajuste de download; `main.tsx` sem `basename`, `api.ts` com `""` em produção, `Dockerfile` sem `ARG VITE_BASE`, sem serviço `reagentes-frontend`. Precisa aplicar o mesmo patch (`Prompt_Encaixe_Juliana.md`, "repositório 2").
...
7. **Anexos em respostas do Mural** — hoje só o post principal aceita anexo; as respostas continuam texto puro.
8. **Busca dentro dos módulos** — a busca global cobre só o que é do Core (Mural, agenda, pessoas, catálogo de módulos); buscar amostra/reagente dependeria de cada módulo expor um endpoint de busca.
